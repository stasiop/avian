...
if ($result = mysqli_query($con, 'SELECT server_users FROM server_nfo WHERE server_invite="' . $_GET['server_invite'] . '"')) {
        ...
        if(explode($scary[0], "[" . $_SESSION['UniversalID'] . "]")){echo "user is already in the server"; $_SESSION['server_selected']=$_GET['server_invite']; header('Location: server_finder.php');}else{
        $result = mysqli_query($con, 'SELECT server_users FROM server_nfo WHERE server_invite="' . $_GET['server_invite'] . '"');
        if($row = mysqli_fetch_row($result)){
        if($new_row=$row[0] . ',[' . (int)$_SESSION['UniversalID'] . ']'){
        if($result = mysqli_query($con, 'UPDATE server_nfo SET server_users="' . $new_row . '" WHERE server_invite="' . $_GET['server_invite'] . '"')){
        $_SESSION['server_selected']=$_GET['server_invite'];
